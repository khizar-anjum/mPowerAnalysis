...
def random_mixture_model(pos_mu=.6,pos_sigma=.1,neg_mu=.4,neg_sigma=.1,size=200):
    pos = [(1,random.gauss(pos_mu,pos_sigma),) for x in range(int(size/2))]
    neg = [(0,random.gauss(neg_mu,neg_sigma),) for x in range(int(size/2))]
    return pos+neg
...
class ROCData(object):
    ...
    def __init__(self,data,linestyle='rx-'):
        ...
        """
        self.data = sorted(data,key = lambda x: cmp(x[1],x[0]))#cmp(y[1],x[1]))
