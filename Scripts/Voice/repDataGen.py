...
for x in v_data.index.levels[0]:
    for i in range(int(len(v_data.loc[x,:].index)/parts)):
        X = feature_extractor(np.load('..\\..\\Data\\VoiceData\\rep20dsData\\X_'+x+'_sec0.npy'),Fs=2205)
        X = np.vstack((X,feature_extractor(np.load('..\\..\\Data\\VoiceData\\rep20dsData\\X_'+x+'_sec'+str(i)+'.npy'),Fs=2205)))
        update_progress(i/v_data.loc[x,:].index/parts)
