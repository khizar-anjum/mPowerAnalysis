...
#%% Lets remove the erroneus samples from the rep_voice_cohort.csv
    #this block is clumsy! hope to better this code soon!
#np.save('..\\..\\Data\\VoiceData\\rep20dsData\\err_samples.npy',err_samples)
err_samples = np.load('..\\..\\Data\\VoiceData\\rep20dsData\\err_samples.npy');
X_test = v_data.loc['test'].reset_index().drop([int(x) for x in err_samples[0:3,1]])
X_train = v_data.loc['train'].reset_index().drop([int(x) for x in err_samples[3:,1]])
X_val = v_data.loc['val'].reset_index()
rp20ds = pd.concat([X_train, X_val,X_test],keys=['train','val','test'])
rp20ds.to_csv('..\\..\\Data\\VoiceData\\rep20dsData\\Seriesrep20dsData.csv')
#v_data.to_csv()
...
#CAUTION: THIS BLOCK IS NOT TESTED!
parts = 500;
X = [];
err_samples = [];
for x in v_data.index.levels[0]:
    for i in range(int(len(v_data.loc[x,:].index)/parts)+1):
        if(i==0):err, X = feature_extractor(np.load('..\\..\\Data\\VoiceData\\rep20dsData\\X_'+x+'_sec0.npy'),Fs=2205)
        else:err, X = np.vstack((X,feature_extractor(np.load('..\\..\\Data\\VoiceData\\rep20dsData\\X_'+x+'_sec'+str(i)+'.npy'),Fs=2205)))
        update_progress((i+1)/int(len(v_data.loc[x,:].index)/parts))
        if(len(err)!=0):err_samples.append((err,X,i));
...
#%% saving the series file for this!
# CAUTION THIS BLOCK HAS ALSO NOT BEEN TESTED!
rfseries = pd.read_csv('..\\..\\Data\\VoiceData\\rep20dsData\\Seriesrep20dsData.csv',index_col = [0,1])
rfseries = rfseries.drop(index=('train',3701)) #derived from err_samples
rfseries.to_csv('..\\..\\Data\\VoiceData\\repRFData\\SeriesrepRFData.csv')
